...
export async function POST(req: NextRequest, {params }: { params: Promise<{ tenantId: string }> }) {
  ...
  try {
    ...
    const item = await db.venue.create({ data: { ...data, tenantId } });
...
export async function PUT(req: NextRequest, {params }: { params: Promise<{ tenantId: string }> }) {
  ...
  try {
    const data = await req.json();
    const item = await db.venue.update({ where: { id: data.id, tenantId }, data });
    return NextResponse.json(item);
  } catch (error: any) { return NextResponse.json({ error: error.message }, { status: 500 }); }
}
...
export async function DELETE(req: NextRequest, {params }: { params: Promise<{ tenantId: string }> }) {
  ...
  try {
    ...
    await db.venue.update({ where: { id, tenantId }, data: { isDeleted: true } });
