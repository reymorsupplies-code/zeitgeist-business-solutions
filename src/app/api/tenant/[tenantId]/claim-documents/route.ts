...
export async function GET(req: NextRequest, {params }: { params: Promise<{ tenantId: string }> }) {
  ...
  try {
    ...
    const items = await db.claimDocument.findMany({ where: whereClause, orderBy: { createdAt: 'desc' } });
...
export async function PATCH(req: NextRequest, {params}: { params: Promise<{ tenantId: string }> }) {
  ...
  try {
    ...
    const item = await db.claimDocument.update({ where: { id, tenantId }, data: updateData });
    return NextResponse.json(item);
  } catch (error: any) { return NextResponse.json({ error: error.message }, { status: 500 }); }
}
...
export async function DELETE(req: NextRequest, {params}: { params: Promise<{ tenantId: string }> }) {
  ...
  try {
    ...
    await db.claimDocument.update({ where: { id: data.id, tenantId }, data: { isDeleted: true } });
