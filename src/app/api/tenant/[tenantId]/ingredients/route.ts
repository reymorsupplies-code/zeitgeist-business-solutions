...
export async function PUT(req: NextRequest) {
  ...
  try {
    const updated = await db.ingredient.update({ where: { id }, data: fields });
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    await db.ingredient.update({ where: { id }, data: { isDeleted: true } });
    return NextResponse.json({ success: true });
  } catch {
    try {
      await pgQuery(`UPDATE "Ingredient" SET "isDeleted" = true, "updatedAt" = NOW() WHERE id = $1`, [id]);
      return NextResponse.json({ success: true });
    } catch (err: any) {
      return NextResponse.json({ error: err.message }, { status: 500 });
    }
