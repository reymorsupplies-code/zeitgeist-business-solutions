...
export async function PUT(req: NextRequest) {
  ...
  try {
    const updated = await db.catalogItem.update({ where: { id, tenantId }, data: whitelistFields('CatalogItem', fields) });
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    await db.catalogItem.update({ where: { id, tenantId }, data: { isDeleted: true } });
    return NextResponse.json({ success: true });
  } catch {
    try {
      await pgQuery(`UPDATE "CatalogItem" SET "isDeleted" = true, "updatedAt" = NOW() WHERE id = $1 AND "tenantId" = $2`, [id, tenantId]);
      return NextResponse.json({ success: true });
    } catch (err: any) {
      return NextResponse.json({ error: err.message }, { status: 500 });
    }
