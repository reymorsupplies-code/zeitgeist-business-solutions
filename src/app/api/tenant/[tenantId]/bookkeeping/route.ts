...
export async function POST(req: NextRequest, {params }: { params: Promise<{ tenantId: string }> }) {
  ...
  try {
    ...
    const entry = await db.bookkeepingEntry.create({
      data: { tenantId, date: data.date ? new Date(data.date) : new Date(), description: data.description, category: data.category, type: data.type || 'debit', amount: data.amount || 0, currency: data.currency || 'TTD', reference: data.reference, accountId: data.accountId }
    });
...
export async function PUT(req: NextRequest) {
  ...
  try {
    const updated = await db.bookkeepingEntry.update({ where: { id, tenantId }, data: whitelistFields('BookkeepingEntry', fields) });
    ...
    try {
      ...
      await pgQuery(`UPDATE "BookkeepingEntry" SET ${setParts.join(', ')} WHERE id = $${pIdx}`, [...paramValues, id]);
      const updated = await pgQueryOne(`SELECT * FROM "BookkeepingEntry" WHERE id = $1`, [id]);
      return NextResponse.json(updated);
    } catch (err: any) {
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    await db.bookkeepingEntry.update({ where: { id, tenantId }, data: { isDeleted: true } });
