...
// GET: List all property units for this tenant (filtered by property.tenantId)
...
// POST: Create a new property unit (verify the property belongs to this tenant)
...
// PUT: Update a property unit (verify unit's property belongs to this tenant)
export async function PUT(req: NextRequest, { params }: { params: Promise<{ tenantId: string }> }) {
  ...
  try {
    ...
    const existing = await db.propertyUnit.findUnique({
      where: { id: data.id },
    ...
    try {
      ...
      const sql = `UPDATE "PropertyUnit" pu SET ${setClauses.join(', ')}
        FROM "Property" p
        WHERE pu.id = $${pIdx++} AND pu."propertyId" = p.id AND p."tenantId" = $${pIdx}
        RETURNING pu.*`;
...
// DELETE: Delete a property unit (verify unit's property belongs to this tenant)
