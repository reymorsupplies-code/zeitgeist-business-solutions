...
export async function PUT(req: NextRequest) {
  ...
  try {
    const updated = await db.designItem.update({ where: { id, tenantId }, data: whitelistFields('DesignItem', fields) });
...
export async function DELETE(req: NextRequest) {
  ...
  if (!id) return NextResponse.json({ error: 'ID required' }, { status: 400 });
  try {
    await db.designItem.update({ where: { id, tenantId }, data: { isDeleted: true } });
    return NextResponse.json({ success: true });
  } catch {
    try {
      await pgQuery(`UPDATE "DesignItem" SET "isDeleted" = true, "updatedAt" = NOW() WHERE id = $1 AND "tenantId" = $2`, [id, tenantId]);
      return NextResponse.json({ success: true });
    } catch (err: any) {
      return NextResponse.json({ error: err.message }, { status: 500 });
    }
