...
export async function PUT(req: NextRequest, {params }: { params: Promise<{ tenantId: string }> }) {
  ...
  try {
    const updated = await db.retailProduct.update({ where: { id, tenantId }, data: whitelistFields('RetailProduct', fields) });
...
export async function DELETE(req: NextRequest, {params }: { params: Promise<{ tenantId: string }> }) {
  ...
  try {
    await db.retailProduct.update({ where: { id, tenantId }, data: { isDeleted: true } });
