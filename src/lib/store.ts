...
export type ViewMode = 'portal' | 'login' | 'control_tower' | 'tenant_app' | 'onboarding' | 'pending_approval' | 'renter_portal' | 'insurance_portal';
...
interface AppState {
  ...
  renterToken: string | null;
  setRenterToken: (token: string | null) => void;
  renterInfo: any | null;
  setRenterInfo: (info: any) => void;
  renterPage: string;
  setRenterPage: (page: string) => void;
...
export const useAppStore = create<AppState>((set) => ({
  ...
  renterToken: (typeof window !== 'undefined' ? localStorage.getItem('zbs-renter-token') : null) as string | null,
  setRenterToken: (renterToken) => {
    if (typeof window !== 'undefined') {
      if (renterToken) localStorage.setItem('zbs-renter-token', renterToken);
      else localStorage.removeItem('zbs-renter-token');
    }
    set({ renterToken });
  },
  renterInfo: null,
  setRenterInfo: (renterInfo) => set({ renterInfo }),
  renterPage: 'rp-dashboard',
  setRenterPage: (renterPage) => set({ renterPage }),
